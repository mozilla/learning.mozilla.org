var TRAVIS_BRANCH_CONFIGS = {
  develop: {
    AWS_BUCKET: 'teach.mofostaging.net',
    NODE_ENV: 'production',
    ORIGIN: 'https://teach.mofostaging.net',
    SHOW_DEV_RIBBON: 'on',
    NEWSLETTER_MAILINGLIST_URL: 'https://sendto.mozilla.org/page/s/mentor-signup-for-teach-site-staging-testing',
    NEWSLETTER_MAILINGLIST_PRIVACY_NAME: 'custom-3557',
    OPTIMIZELY_ID: '206878104',
    OPTIMIZELY_ACTIVE: 'yes',
    MAKE_METADATA_URL: 'https://{username}.mywebmaker.org/makes.json'
  },
  master: {
    AWS_BUCKET: 'teach-mozilla-org-s3bucket-1oinic8tfxxim',
    NODE_ENV: 'production',
    TEACH_API_URL: 'https://teach-api-production.herokuapp.com',
    ORIGIN: 'https://teach.mozilla.org',
    NEWSLETTER_MAILINGLIST_URL: 'https://sendto.mozilla.org/page/s/mentor-signup',
    NEWSLETTER_MAILINGLIST_PRIVACY_NAME: 'custom-1788',
    OPTIMIZELY_ID: '206878104',
    OPTIMIZELY_ACTIVE: 'yes',
    MAKE_METADATA_URL: 'https://{username}.makes.org/makes.json'
  }
};

exports.getS3Env = function(env) {
  env = env || process.env;

  var config = TRAVIS_BRANCH_CONFIGS[env.TRAVIS_BRANCH];

  if (env.TRAVIS !== 'true' || env.TRAVIS_PULL_REQUEST !== 'false' ||
      !config) {
    return null;
  }

  return config;
};
