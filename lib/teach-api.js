...
var emulateLogin = process.env.get('LOGIN_EMULATION', false);
var emulateLoggedIn = process.env.get('LOGIN_EMULATION_LOGGEDIN', false);
var emulateUsername = process.env.get('LOGIN_EMULATION_USERNAME', 'testuser');
...
_.extend(TeachAPI.prototype, {
  ...
  getUsername: function() {
    if (emulateLogin) {
      return emulateLoggedIn ? emulateUsername : null;
    }
  ...
  checkLoginStatus: function(onLoginStatusChecked) {
    ...
    if (emulateLogin) {
      var body = { username: emulateLoggedIn ? emulateUsername : null };
  ...
  request: function(method, path) {
    if (emulateLogin) {
      return { accept: function() { return { end: function(fn) { fn(false, { body: ''}); }}}};
    }
